...
describe Vulture do
  ...
  context '.to_analize' do
    it 'perform a analize for a specific category AND find a RCE' do
      allow_any_instance_of(Vulture).to receive(:new).and_return(Vulture)
      vulture = Vulture.new({:rot => 'rce',:lang => 'php',})
      vulture.patterns = vulture.get_patterns('rce')
      ...
      expect(result.first).to have_key(:matched)
      expect(result.first).to have_key(:line_number)
