...
describe Vulture::Util do
    before(:context) do
        RSpec::Mocks.with_temporary_scope do
            allow_any_instance_of(Vulture).to receive(:new).with(be_kind_of(Hash)).and_return(Vulture.new({}))
            @vulture = Vulture.new {}
        end
    end
    context '.get_manipulable_inputs' do
        it 'invalid file path' do
            expect{ @vulture.get_manipulable_inputs('noexit_path','php') }.to raise_error(RuntimeError)
        end
